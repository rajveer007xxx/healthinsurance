#!/usr/bin/env python3
"""
Test renewal script for prepaid and postpaid customers
"""
...
def test_renewal_for_customer(customer_id, db):
    """Test renewal for a specific customer"""
    try:
        ...
        print(f"\n{'='*80}")
        print(f"TESTING {customer.billing_type} CUSTOMER")
        print(f"{'='*80}")
        print(f"Customer: {customer.full_name} ({customer.customer_id})")
        print(f"Email: {customer.email}")
        print(f"Billing Type: {customer.billing_type}")
        ...
        invoice_date = customer.end_date
        due_date = customer.end_date + timedelta(days=7)
        ...
        if customer.billing_type == "PREPAID":
            ...
            print(f"\nPREPAID BILLING PERIOD (FUTURE):")
            print(f"  Period Start: {period_start.strftime('%d-%m-%Y')}")
            print(f"  Period End: {period_end.strftime('%d-%m-%Y')}")
        else:
            period_end = customer.end_date
            period_start = customer.end_date - timedelta(days=period_months * 30)
            print(f"\nPOSTPAID BILLING PERIOD (PAST):")
            print(f"  Period Start: {period_start.strftime('%d-%m-%Y')}")
            print(f"  Period End: {period_end.strftime('%d-%m-%Y')}")
        ...
        print(f"  New end date: {new_end_date.strftime('%d-%m-%Y')}")
        ...
        new_invoice = Invoice(
            ...
            status="SENT",
            sent_type="MANUAL",
            remarks=f"Test renewal - {customer.billing_type}"
        )
        db.add(new_invoice)
        ...
        if customer.email:
            try:
                ...
                print(f"Sending email to RAJVEERSINGH007BOND@GMAIL.COM...")
                send_invoice_email(
                    to_email='RAJVEERSINGH007BOND@GMAIL.COM',
                ...
                print(f"✅ Invoice sent successfully to RAJVEERSINGH007BOND@GMAIL.COM")
                return True
...
def main():
    ...
    try:
        print("="*80)
        print("TESTING PREPAID AND POSTPAID BILLING PERIODS")
        print("="*80)
        
        prepaid_success = test_renewal_for_customer(2, db)
        
        postpaid_success = test_renewal_for_customer(3, db)
        
        print(f"\n{'='*80}")
        print("TEST SUMMARY")
        print(f"{'='*80}")
        print(f"Prepaid Customer (Rajesh Patel): {'✅ SUCCESS' if prepaid_success else '❌ FAILED'}")
        print(f"Postpaid Customer (Priya Kumar): {'✅ SUCCESS' if postpaid_success else '❌ FAILED'}")
        print(f"\nBoth invoices sent to: RAJVEERSINGH007BOND@GMAIL.COM")
        print(f"{'='*80}")
