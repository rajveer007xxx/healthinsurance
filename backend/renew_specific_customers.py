#!/usr/bin/env python3
"""
Renew specific customers and send invoices
"""
...
def renew_customer(customer_id, db):
    """Renew a specific customer"""
    try:
        ...
        print(f"\n{'='*80}")
        print(f"RENEWING {customer.billing_type} CUSTOMER")
        print(f"{'='*80}")
        ...
        new_invoice = Invoice(
            ...
            status="SENT",
            sent_type="MANUAL",
            remarks=f"Manual renewal - {customer.billing_type}"
        )
        db.add(new_invoice)
...
def main():
    ...
    try:
        print("="*80)
        print("RENEWING INVOICES FOR RAJVEER S. AND RAJESH PATEL")
        print("="*80)
        
        rajveer_success = renew_customer(1, db)
        
        rajesh_success = renew_customer(2, db)
        
        print(f"\n{'='*80}")
        print("RENEWAL SUMMARY")
        print(f"{'='*80}")
        print(f"RAJVEER S. (POSTPAID): {'✅ SUCCESS' if rajveer_success else '❌ FAILED'}")
        print(f"Rajesh Patel (PREPAID): {'✅ SUCCESS' if rajesh_success else '❌ FAILED'}")
